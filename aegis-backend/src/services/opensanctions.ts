...
function labelDataset(d: string): string {
...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    if (attempt > 0) {
      ...
      logger.warn({ attempt, url }, 'OpenSanctions: retry');
    }
    ...
    try {
      ...
      if (err.name !== 'AbortError' && err.code !== 'ECONNREFUSED') break;
    }
  }
...
export async function screenEntity(
  name: string,
  country?: string,
  schema?: 'Organization' | 'Person' | 'Vessel',
): Promise<ScreeningResult> {
  const t0 = Date.now();
  logger.info({ name, country }, 'OpenSanctions: screening');

  const params = new URLSearchParams({ q: name, limit: '10', dataset: 'default' });
  if (country) params.set('countries', country.toLowerCase());
  ...
  try {
    ...
    if (!res.ok) {
      logger.warn({ status: res.status }, 'OpenSanctions: non-200');
    ...
    logger.info({ name, hits: hits.length, isSanctioned, latencyMs, retries }, 'OpenSanctions: done');
    ...
    const latencyMs = Date.now() - t0;
    logger.error({ err: err.message, name, latencyMs }, 'OpenSanctions: failed');
...
export async function screenMultiple(
  entities: Array<{ name: string; country?: string; role: string }>,
): Promise<Array<ScreeningResult & { role: string }>> {
