...
exports.analyzeCheck = analyzeCheck;
exports.extractEntitiesFromDocument = extractEntitiesFromDocument;
const openai_1 = __importDefault(require("openai"));
const config_1 = require("../config");
const logger_1 = require("../logger");
const client = new openai_1.default({
    apiKey: config_1.config.DEEPSEEK_API_KEY,
...
function buildPrompt(f, sanctionsContext) {
...
async function analyzeCheck(formData, sanctionsContext = null) {
    logger_1.logger.info({ counterparty: formData.cp }, 'DeepSeek analysis started');
    ...
    logger_1.logger.info({ verdict: result.verdict, score: result.score }, 'DeepSeek analysis complete');
    return result;
}
// Extract entities from a document (for document analysis feature)
async function extractEntitiesFromDocument(text) {
    const response = await client.chat.completions.create({
        model: 'deepseek-chat',
        max_tokens: 512,
        response_format: { type: 'json_object' },
