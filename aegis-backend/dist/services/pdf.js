...
exports.generateCompliancePDF = generateCompliancePDF;
...
async function generateCompliancePDF(check) {
    logger_1.logger.info({ checkId: check.id }, 'Generating compliance PDF');
    return new Promise((resolve, reject) => {
        ...
        for (let i = pages.start; i < pages.start + pages.count; i++) {
            ...
            doc.fillColor(COLORS.gray).fontSize(7).font('Helvetica')
                .text(`Aegis Comply · aegis-comply.ru · ID: ${check.id} · ${check.createdAt.toISOString()} · Page ${i + 1} of ${pages.count}`, 50, doc.page.height - 48, { width: pageWidth, align: 'center' });
            doc.fillColor(COLORS.gray).fontSize(7)
                .text('DISCLAIMER: Данный отчёт сформирован AI-системой Aegis Comply как справочный материал. Не является юридическим заключением. Финальное решение — за комплаенс-офицером.', 50, doc.page.height - 34, { width: pageWidth, align: 'center' });
        }
        doc.end();
    });
}
